...
export NV_TAO_TF_TOP="$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd )"

function tao_tf {
   python $NV_TAO_TF_TOP/runner/tao_tf.py "$@"
}
export -f tao_tf
...
function _check_tao_tf1_requirements(){
    warnings=()
...
_check_tao_tf1_requirements || echo -e "\033[1;31mBuild environment setup failed.\033[0m"
