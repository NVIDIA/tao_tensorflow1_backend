...
from modulus.processors.augment.spatial import random_zoom_matrix
...
class TestRandomZoom(ProcessorTestCase):
    @parameterized.expand(
        [
            [
                -0.1,
                _esc(
                    "RandomZoom.probability (-0.1) is not within the range [0.0, 1.0]."
                ),
            ],
            [
                1.1,
                _esc(
                    "RandomZoom.probability (1.1) is not within the range [0.0, 1.0]."
                ),
            ],
        ]
    )
    def test_raises_on_invalid_probability(self, probability, message):
        with self.assertRaisesRegexp(ValueError, message):
            RandomZoom(probability=probability)

    @patch(
        "modulus.processors.augment.spatial.random_zoom_matrix",
        side_effect=random_zoom_matrix,
    )
    def test_delegates_to_random_zoom_matrix(self, spied_random_zoom_matrix):
        example = self.make_example_128x240()
        augmentation = RandomZoom(ratio_min=0.1, ratio_max=2.0, probability=1.0)
        augmentation.process(example)
        spied_random_zoom_matrix.assert_called_with(
            ratio_min=0.1, ratio_max=2.0, width=240, height=128, batch_size=None
        )

    def test_serialization_and_deserialization(self):
        """Test that it is a TAOObject that can be serialized and deserialized."""
        augmentation = RandomZoom(ratio_min=1, ratio_max=5, probability=1.0)
